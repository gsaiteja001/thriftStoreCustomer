...
useEffect(() => {
  const fetchAddress = async () => {
    try {
      const response = await fetch(`https://thriftstorebackend.onrender.com/api/address/${addressId}`);
      if (!response.ok) {
...
const createNewAddress = async (addressData) => {
  const addressId = localStorage.getItem('addressID1');
  try {
    const response = await axios.post(`https://thriftstorebackend.onrender.com/api/address/${addressId}`, addressData);
...
const updateAddress = async (addressId, addressData) => {
  try {
    const response = await axios.put(`https://thriftstorebackend.onrender.com/api/address/${addressId}`, addressData);
...
const deleteAddress = async (addressId) => {
  try {
    const response = await axios.delete(`https://thriftstorebackend.onrender.com/api/address/${addressId}`);
