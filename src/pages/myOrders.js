...
const MyOrders = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await axios.get(`https://kissan-backend-1.onrender.com/api/orders/${customer_id}`);
  ...
  const fetchItemDetails = async (ordersArray) => {
    ...
    try {
      const details = await Promise.all(
        itemIds.map((id) =>
          axios.get(`https://kissan-backend-1.onrender.com/api/item/${id}`).then((res) => res.data)
        )
      );
